...
def test_pending_journal_authorizes_idempotent_creation_then_commit(tmp_path):
...
def test_transaction_request_journals_before_publishing_generation(tmp_path):
    paths = build_paths(tmp_path / "db"); setup_database(paths["root"])
    identity = __import__('json').loads(paths["identity"].read_text())["database-id"]
